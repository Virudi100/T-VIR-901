...
public class BulletRebond : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!rebonded)  //Necessaire pour empecher le laser de rebondir sur l'autre face après avoir traversé le prisme
        {
            ...
            {
                gameObject.GetComponent<Rigidbody>().AddForce(Vector3.up * other.transform.parent.gameObject.GetComponent<IndiceDeRefraction>().iDR * 50);      //Ajoute une force vers le haut influencer par l'indice de refraction
            ...
            {
                gameObject.GetComponent<Rigidbody>().AddForce(Vector3.up * other.transform.parent.gameObject.transform.parent.GetComponent<IndiceDeRefraction>().iDR * 50);     //Ajoute une force vers le haut influencer par l'indice de refraction
